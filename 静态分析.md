...
1. ​**任务生成**​：只有当前任务遇到未定义的函数或变量（即代码中未提供其实现或定义，无法直接分析）时，才生成子任务来分析这些元素。否则，直接基于当前代码判断风险。 同一个函数在不同地方调用，要生成不同的任务。
3. ​**修正机制**​：当前任务的分析可能修正父任务或祖先任务的结果。例如，如果子任务发现参数未验证，就修正父任务的风险为存在（如命令注入），否则标记为安全。
4. ​**白名单函数**​：函数名匹配时，不生成子任务（注意：仅仅是不生成子任务）。只匹配函数名部分，如：obj.myfunc()匹配的是myfunc。白名单函数列表：getScriptPath, runCommand。
5. ​**历史任务上下文**​：每个任务都携带所有祖先任务的信息（以任务ID和简要描述形式），以便参考和修正。历史任务栈以列表形式提供。
...
* `项目上下文信息`: 与项目相关的各种信息。
* `代码信息`: 当前分析的Java代码片段。
* `历史任务列表`: 历史任务列表，每个任务包括任务ID、描述和当前状态（例如，{"task_id": "T1", "description": "分析main函数的安全风险", "status": "pending"}）。历史任务栈从根任务开始，按顺序列出。
* `当前任务`: 当前任务的对象，包括任务ID、描述和任何相关上下文。
...
```
{
  ...
  "sub_tasks": [
    {
      "task_id": "T1.1",
      "description": "分析第3行getUserInput函数的安全实现：重点关注它的返回值，因为返回值用于后续的命令拼接",
      ...
      "analysis_focus": "1) 重点关注它的返回值，因为返回值用于后续的命令拼接",

"suggest_tasklist": "T1"
    },
    {
      "task_id":"T1.2",
      "description":"分析第4行getBasePath函数的安全实现：重点关注它的返回值，因为返回值用于后续的路径拼接",
      "impact_level":"中",
      ...
      "analysis_focus":"1) 重点关注它的返回值，因为返回值用于后续的路径拼接",
